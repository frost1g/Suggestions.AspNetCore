...
namespace Suggestions.AspNetCore.Tests
{
    [TestFixture]
    public class SuggestionsClientTest
    {
        ...
        {
            var query = "Москва, Турчанинов пер, д 6 стр 2";
        ...
        {
            var query = new PartySuggestQuery("Виташевский Игорь");
            query.type = PartyType.INDIVIDUAL;
            var response = api.QueryParty(query);
            Assert.AreEqual("550608718785", response.suggestions[0].data.inn);
            Console.WriteLine(string.Join("\n", response.suggestions));
        }

        [Test]
        public void SuggestCountryTest()
        {
            var query = "Росс";
            var response = api.QueryCountry(query);
            Assert.AreEqual("Россия", response.suggestions[0].value);
            Console.WriteLine(string.Join("\n", response.suggestions));
        }

        [Test]
        public void SuggestCurrencyTest()
        {
            var query = "Евр";
            var response = api.QueryCurrency(query);
            Assert.AreEqual("Евро", response.suggestions[0].value);
            Console.WriteLine(string.Join("\n", response.suggestions));
        }
    }
}
