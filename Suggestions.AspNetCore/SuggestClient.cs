using RestSharp;
using Suggestions.AspNetCore.Query;
...
namespace Suggestions.AspNetCore
{
    public class SuggestClient
    {
        ...
        const string COUNTRY_RESOURCE = "country";
        const string CURRENCY_RESOURCE = "currency";
        ...
        public SuggestCountryResponse QueryCountry(string country)
        {
            return QueryCountry(new CountrySuggestQuery(country));
        }

        public SuggestCountryResponse QueryCountry(CountrySuggestQuery query)
        {
            var request = new RestRequest(COUNTRY_RESOURCE, Method.POST);
            return Execute<SuggestCountryResponse>(request, query);
        }

        public SuggestCurrencyResponse QueryCurrency(string currency)
        {
            return QueryCurrency(new CurrencySuggestQuery(currency));
        }

        public SuggestCurrencyResponse QueryCurrency(CurrencySuggestQuery query)
        {
            var request = new RestRequest(CURRENCY_RESOURCE, Method.POST);
            return Execute<SuggestCurrencyResponse>(request, query);
        }
